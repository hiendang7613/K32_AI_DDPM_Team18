...
def resize_and_rescale(img, img_shape, clip_min, clip_max, **kwargs):
    ...
    img = tf.image.resize(img, size=(img_shape[0], img_shape[1]), antialias=True)
...
def train_preprocessing(x):
    img = x["image"]
    img = resize_and_rescale(img, **dataset_cfg, )
    img = augment(img)
    return img
...
def build_model(
    ...
    for i in range(len(widths)):
        ...
        if i != len(widths)-1: #widths[i] != widths[-1]:
...
class DiffusionModel(keras.Model):
    def __init__(self, network, ema_network, timesteps, gdf_util, img_shape, ema=0.999):
        super().__init__()
    ...
    def generate_images(self, num_images=16):
        ...
        samples = tf.random.normal(shape=(num_images, self.img_shape[0], self.img_shape[1], self.img_shape[2]), dtype=tf.float32)
...
## Dataset
def GetDataset(batch_size, dataset_name, splits, data_dir, **kwargs):

    (ds,) = tfds.load(dataset_name, split=splits, with_info=False, shuffle_files=True, try_gcs = True, data_dir=data_dir, download=False)
...
if __name__=='__main__':
    ...
    train_ds = GetDataset(batch_size, **dataset_cfg)

    model = createModel(strategy, dataset_cfg['img_shape'], **model_cfg)

    # Compile the model
    model.compile(
        loss=keras.losses.MeanSquaredError(reduction=tf.keras.losses.Reduction.SUM_OVER_BATCH_SIZE),
